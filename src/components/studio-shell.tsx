...
import { Disc3, FolderOpen, ListMusic, Mic, Settings2, SlidersHorizontal } from "lucide-react";
    ...
    <div className="flex min-h-dvh flex-col bg-bg text-fg">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <main
          className={cn(
            "min-w-0 flex-1",
            studioHome ? "flex min-h-0 flex-col overflow-hidden pb-28 md:pb-24" : "overflow-y-auto pb-28 md:pb-24",
          )}
        >
          {children}
        </main>
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.to}
              className={cn(
                "flex h-14 flex-1 flex-col items-center justify-center gap-1 text-xs uppercase tracking-wider",
