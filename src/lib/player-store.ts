...
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  setVocalGain: (v) => set({ vocalGain: v }),
}));
...
export function hasStemMix(track: TrackPublic | null): boolean {
  return Boolean(track?.vocal_url && track?.instrumental_url);
}
